...
impl AlertsProvider {
    pub fn new(config: Arc<AppConfig>) -> Result<Self> {
        Ok(Self { config, client: Client::builder().build()? })
    }
    ...
    pub async fn fetch(&self) -> Result<FetchResult> {
        ...
        if !http_status.is_success() {
            ...
            let next = if http_status.as_u16() == 429 { config.rate_limit_backoff_seconds } else { config.poll_interval_seconds };
            bail!("alerts.in.ua повернув HTTP {}: {} [next_poll={next}]", http_status.as_u16(), body);
        }

        let payload = response.json::<String>().await
            .map_err(|e| anyhow!("alerts.in.ua повернув некоректну JSON-відповідь: {e}"))?;
        if !matches!(payload.as_str(), "A" | "P" | "N") { bail!("alerts.in.ua повернув невідомий статус тривоги"); }
        let active = payload == "A" || (payload == "P" && config.partial_status_is_active());
        Ok(FetchResult {
            status: AlertStatus { active, checked_at: Utc::now(), matched_uids: if active { vec![config.location_uid] } else { Vec::new() } },
